        ...
        execute: function () {
            ...
            _export("VPcontrolPanel", VPcontrolPanel = function (_RODIN$Sculpt) {
                ...
                function VPcontrolPanel(_ref) {
                    ...
                    _this.esiminch = new RODIN.Box(_this.width, 2 * _this.width / 3, .1, new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }));
                    _this.esiminch.on(RODIN.CONST.READY, function () {
                        _this.esiminch.parent = _this.panel;
                        _this.esiminch.position.set(0, 0, -.1);
                        _this.esiminch.hoverOutTime = Infinity;
                        _this.esiminch.hasCloseAction = false;
                    });

                    _this.esiminch.on(RODIN.CONST.GAMEPAD_HOVER, function () {
                        _this.esiminch.hoverOutTime = Infinity;
                    });

                    _this.esiminch.on(RODIN.CONST.GAMEPAD_HOVER_OUT, function () {
                        _this.esiminch.hoverOutTime = RODIN.Time.now;
                    });

                    _this.esiminch.on(RODIN.CONST.UPDATE, function () {
                        if (RODIN.Time.now - _this.esiminch.hoverOutTime > 3000 && !_this.esiminch.hasCloseAction) {
                            _this.hideControls();
                            _this.esiminch.hasCloseAction = true;
                        }
                    });
